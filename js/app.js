...
function mostrarPokemon(objeto_pokemon) {
  let tipos = objeto_pokemon.types.map(
    (type) => `<p class="${type.type.name} tipos">${type.type.name}</p>`
  );
  tipos = tipos.join("");
  // console.log(tipos);
  ...
  div.innerHTML = `
    <div class="poke-card">
        ...
        <div class="poke-id-nombre">
            <p class="poke-id"># ${objeto_pokemon.id}</p>
            <p class="poke-nombre"> ${objeto_pokemon.name} </p>
        </div>
        <img src="${objeto_pokemon.sprites.other["official-artwork"].front_default}" alt="${objeto_pokemon.name}" srcset="" class="poke-img">
        <div class="poke-tipo">
            ${tipos}
        </div>
        <div class="poke-datos">
            <p class="tipo">${objeto_pokemon.weight} kg</p>
            <p class="tipo">${objeto_pokemon.height} m</p>
        </div>
    </div>
`;
...
btn_buscador.addEventListener("click", () => {
  ...
  if (!encontrado) {
    ...
    setTimeout(() => {
      elemento_error.remove("mensaje-error");
    }, 2000);
  }
});
